  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-[var(--border)] p-8 space-y-6">
        ...
        {/* 썸네일 URL */}
        <div>
          <label className="block text-sm font-bold text-[var(--primary)] mb-2">썸네일 이미지 URL</label>
          <input
            type="url"
            value={form.thumbnail}
            onChange={(e) => setForm({ ...form, thumbnail: e.target.value })}
            placeholder="https://images.unsplash.com/..."
            className="w-full px-4 py-3 border border-[var(--border)] rounded-xl focus:outline-none focus:border-[#A67C52] text-sm font-medium font-mono"
          />
          {form.thumbnail && (
